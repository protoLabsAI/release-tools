 ...
 */
/**
 * The biome version the fleet tracks together. Single source of truth: bump it
 * here and `verify-workspace-config` flags every JS/TS repo that hasn't followed,
 * so the fleet upgrades biome in lockstep instead of drifting per-repo.
 */
export const FLEET_BIOME_VERSION = '2.4.16';

/** Strip a semver range prefix (^ ~ >= <= = and whitespace) → the bare version. */
...
/**
 * The declared @biomejs/biome version spec from a parsed package.json, or null
 * if the repo doesn't depend on biome. Checks devDependencies then dependencies.
 */
export function biomeVersion(pkg) {
  if (!pkg || typeof pkg !== 'object') return null;
  return pkg.devDependencies?.['@biomejs/biome'] ?? pkg.dependencies?.['@biomejs/biome'] ?? null;
}
...
export const WORKSPACE_STANDARD = /** @type {Rule[]} */ ([
  ...
  {
    id: 'biome-linter',
    description: `JS/TS repos lint with biome ${FLEET_BIOME_VERSION} — the fleet tracks one biome version together (no ESLint, no per-repo version drift)`,
    // warn (not error) so adoption rolls out across the fleet without breaking CI
    // on day one — same posture as workflow-security-lint. Tighten to error once
    // every node repo is on biome. N/A for non-node repos (Python/Rust have no
    // package.json) → ok=true so they don't show as violations.
    severity: 'warn',
    ok: (m) => {
      if (!m.packageJson) return true;
      const v = biomeVersion(m.packageJson);
      if (!v) return false;
      if (!m.hasFile('biome.json') && !m.hasFile('biome.jsonc')) return false;
      return cleanVersion(v) === cleanVersion(FLEET_BIOME_VERSION);
    },
    fix: `Track the fleet biome: \`npm i -D @biomejs/biome@${FLEET_BIOME_VERSION}\`, add a biome.json, set \`"lint": "biome lint ."\` (drop ESLint). Fleet version is pinned in release-tools FLEET_BIOME_VERSION.`,
    detail: (m) => {
      if (!m.packageJson) return [];
      const v = biomeVersion(m.packageJson);
      if (!v) return ['no @biomejs/biome dependency (still on ESLint, or unlinted?)'];
      if (!m.hasFile('biome.json') && !m.hasFile('biome.jsonc'))
        return [`@biomejs/biome ${v} declared but no biome.json / biome.jsonc`];
      if (cleanVersion(v) !== cleanVersion(FLEET_BIOME_VERSION))
        return [`biome ${v} — fleet tracks ${FLEET_BIOME_VERSION}`];
      return [];
    },
  },
]);
